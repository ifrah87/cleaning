 ...
 *
 * Run:  NODE_PATH="$(pwd)/scraper/node_modules" node tests/somebody-can-be-marked-sick.js
 *
...
(async () => {
  ...
  console.log('\n\x1b[1mThe office can actually tell the app somebody is sick\x1b[0m');

  // THE REGRESSION THAT STARTED THIS. cycleStatus existed, did everything, and was
  // called from nowhere — so no screen could set an attendance status at all.
  const reachable = await page.evaluate(() => {
    const src = document.documentElement.innerHTML;
    return { defined: typeof cycleStatus === 'function' };
  });
  check('cycleStatus exists', reachable.defined);

  await page.evaluate(() => { state.tab = 'board'; render(); });
  await page.waitForTimeout(500);

  // Hodan did not badge in, so she is in the NOT IN half of the strip.
  const strip = await page.evaluate(() => {
    const chips = [...document.querySelectorAll('.bd-panel div')]
      .filter((e) => /Hodan/.test(e.textContent) && e.onclick);
    return { found: chips.length, text: chips.map((c) => c.textContent.trim()) };
  });
  check('her name is on the board strip and is tappable', strip.found > 0, JSON.stringify(strip));

  // One tap = late. Four = sick.
  ...
  check('she can be marked SICK', whenSick.status === 'sick' && whenSick.away, JSON.stringify(whenSick));
  check('...and her unfinished rooms are released', whenSick.stillHolding.length === 0,
    JSON.stringify(whenSick.stillHolding));

  const shown = await page.evaluate(() => {
    render();
    return [...document.querySelectorAll('.bd-panel')].map((e) => e.textContent).join(' | ');
  });
  check('the board says SICK against her name', /Hodan/.test(shown) && /SICK/.test(shown),
    shown.slice(0, 200));
